...
# ╔═╡ 11d47dd9-c83c-4e6e-8eb9-ce6c2b1033da
using Latexify
...
# ╔═╡ a00007f2-6b7b-4dbf-aeb7-f622af0b086e
using Flux: crossentropy, Momentum

# ╔═╡ 0385e76e-c2b5-464a-8ca8-3a74543127d6
TableOfContents()

# ╔═╡ 4f3661d4-fea1-4873-88fc-7bcfaccb5046
md"""
## Training a classifier in `Flux`
Getting a real classifier to work might help cement the workflow a bit more.
[MNIST](https://www.cs.toronto.edu/~kriz/mnist.html) is a dataset of 60k small training images of handwritten digits from 0 to 9.
...
!!! note
    *You'll need to load the ```CIFAR10``` dataset instead of the ```MNIST``` one*
"""

# ╔═╡ b83e1d60-6c55-463a-b575-f72efb7a89e1
md"""
Set an environment variable to stop asking approval to download data
"""

# ╔═╡ d29e8665-d43b-4fe7-a60c-2602b3a76d51
ENV["DATADEPS_ALWAYS_ACCEPT"] = true

# ╔═╡ e00fc925-b57c-48b9-ba62-49c9f51d8fdc
begin
	# train_x, train_y = MNIST.traindata(Float32)# or if this not working, you can download it from and load it locally: https://www.cs.toronto.edu/~kriz/cifar-10-binary.tar.gz
	train_x, train_y = CIFAR10(split=:train)[:]      # download the CIFAR-10 training dataset
	# train_x = reshape(train_x, 32, 32, 3, :) 
	# labels2 = onehotbatch(train_y, 0:9)
	labels2 = onehotbatch(train_y, 0:9)
end

# ╔═╡ 49e7f528-306e-46a3-8861-d413e3fb4e5a
md"""
The train\_x array contains 60,000 28x28 pixel images converted to 28 x 28 x 1 arrays with the third dimension acting as a grey-scale channel. Let's take a look at a random image from train_x. For this, we need to convert the floats in the 28x28 matrix to the grey colour value:
!!! note
    *Use the ```colorview``` function to display the image*
"""

# ╔═╡ 49db67b8-60bd-461a-9e37-16713dfda95d
begin
	# Gray.(permutedims(reshape(train_x[:,:,:,rand(1:end)], 32, 32), (2,1)))
	ximg = colorview(RGB, permutedims(train_x[:,:,:,rand(1:end)], (3, 1, 2)))
	plot(ximg)									# Use colorview to show the images
end
...
!!! note
	*The 50,000 ```CIFAR10``` images comprise 32 x 32 x 3 arrays with the third dimension corresponding to the 3 colour channels (R,G,B).  You'll need to take account of this difference in the structure of the network.*
"""

# ╔═╡ 4477517d-9990-48bf-b247-a5c83d294c33
begin
	train = ([(train_x[:,:,:,i], labels2[:,i]) for i in partition(1:49000, 1000)]) |> 	gpu
	valset = 49001:50000 			# CIFAR-10 training dataset is size of 50000
	valX = train_x[:,:,:,valset] |> gpu
	valY = labels2[:, valset] |> gpu
end
...
!!! note
	*Both the structure of the first ```Conv``` layer and the first ```Dense``` layer will need to be adjusted to take account of the different shape of the input images.*
"""
...
#   Dense(84, 10),
#   softmax) |> gpu
m3 = Chain( 								# Recrearting a CNN model
  Conv((5,5),  3=>16, relu), 
  MaxPool((2,2)),            		
  Conv((5, 5), 16=>8, relu),
  MaxPool((2, 2)),           		
...
begin
	loss2(x, y) = sum(crossentropy(m3(x), y))
	opt2 = Momentum(0.01)
end
...
accuracy(x, y) = mean(onecold(m3(x), 0:9) .== onecold(y, 0:9))
...
!!! note
	*Because the images are more complex than the black and white ```MNIST``` images, you might want to train for more epochs.  After about 10 epochs you'll get close to 50% accuracy, which less than the 98% you get for ```MNIST```, but better than the 10% you'd expect based on chance.*
"""
...
begin
	...
	for epoch = 1:epochs
  		for d in train
    		gs = gradient(Flux.params(m3)) do
      			l = loss2(d...)
    		end
    		update!(opt2, Flux.params(m3), gs)
		end
  		push!(train_acc, accuracy(valX, valY))
		push!(train_epochs, epoch)
	end
end

# ╔═╡ 4a809288-9451-4677-a520-1ae3165d84a6
begin
	plot(train_epochs, train_acc, lab="")
	yaxis!("Accuracy")
...
!!! note
	*Change the source of the test data and its dimensions.*

"""

# ╔═╡ dc39aa27-0ed1-4676-a2ed-087a9f9d1c6f
begin
	test_x, test_y = CIFAR10(split=:test)[:] 		# download CIFAR-10 testdata
	# test_x = reshape(test_x, 32, 32, 3, :)
end

# ╔═╡ ac32ec6d-9690-4e23-846c-5f9b98d934a1
test_labels = onehotbatch(test_y, 0:9)

# ╔═╡ f012dd30-bd74-4b5f-b30b-34b35c0bc608
test = gpu.([(test_x[:,:,:,i], test_labels[:,i]) for i in partition(1:10000, 1000)])

# ╔═╡ 0624ab1b-3eb2-46bc-9647-d46ed1c10ad3
md"""
Next, display images from the test set.
"""

# ╔═╡ 742abbf5-3f9f-4cc5-8aa6-a00ea83571e5
@bind image_index Slider(1:100:10000, default=1)

# ╔═╡ a2470504-1a08-40aa-a03e-e10073334f0b
begin
	# Gray.(permutedims(reshape(test_x[:,:,:,image_index], 32, 32), (2,1)))
	timg = colorview(RGB, permutedims(test_x[:,:,:,image_index], (3, 1, 2)))
	plot(timg)							# Use colorview to show the test images
end
...
# ╔═╡ b445271d-a904-490e-b640-7d08764a5530
begin
	ids = rand(1:10000, 5)
...
manifest_format = "2.0"
project_hash = "c56805ea16e790716b5366d8adf7f589e1b4235d"
...
git-tree-sha1 = "56fcd2974239c7703cb5d0154c3a7351f24d264c"
